  ...
  return (
    <div className="panel p-3">
      <div className="flex items-center gap-1.5 px-1 pb-2 text-[10px] tracking-[0.16em] font-bold text-[var(--violet)]">
        ...
        <span className="ml-auto mono text-[var(--faint)] normal-case tracking-normal text-[10px]">
            ...
            {[...zones].reverse().map((z) => (
              <tr key={z.id} className="border-b border-[var(--line-soft)]">
                ...
                <td className="text-[var(--muted)]">
                  {z.flipped === "UP" ? "broken → support" : z.flipped === "DOWN" ? "broken → resistance" : z.retested ? "retested" : ""}
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div className="panel p-3 overflow-x-auto">
            ...
            return (
              ...
              >
                ...
                <td className={`text-right pr-3 ${signColor(s.ceOiChg)}`}>
                  {s.ceOiChg != null ? fmtSigned(s.ceOiChg / 1000, 1, "K") : "—"}
                </td>
                ...
                <td className={`text-left pl-3 ${signColor(s.peOiChg)}`}>
                  {s.peOiChg != null ? fmtSigned(s.peOiChg / 1000, 1, "K") : "—"}
                </td>
      ...
      <div className="mt-2 mono text-[9.5px] text-[var(--faint)] px-1">
        {hasBaseline
          ? "ΔOI vs today's first chain capture (real session baseline). Bars scaled to the heaviest strike in view."
          : "ΔOI pending — the session baseline is captured on the first chain fetch of the day. Bars scaled to the heaviest strike in view."}
      </div>
    </div>
  );
}
    ...
    <div className="max-w-[1720px] mx-auto px-3 sm:px-5 pb-16">
      <header className="sticky top-0 z-40 -mx-3 sm:-mx-5 px-3 sm:px-5 py-2.5 border-b border-[var(--line)] backdrop-blur-md" style={{ background: "rgba(4,6,11,.82)" }}>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-[var(--faint)]">{d?.name ?? ""}</span>
          </div>
      ...
      {d && (
        <div className="mt-4 grid grid-cols-12 gap-4">
          {/* price row */}
          <div className="col-span-12 panel px-5 py-4 flex items-end gap-6 flex-wrap">
            <div>
              <div className="label">LAST TRADED PRICE</div>
              <div className="flex items-baseline gap-3">
                <span className="mono text-[34px] font-bold leading-none">₹{fmtINR(d.row?.ltp ?? d.card?.ltp ?? null)}</span>
          ...
          {/* chart */}
          <div className="col-span-12 xl:col-span-8 panel p-2">
            <div className="flex items-center gap-2 px-2 pt-1.5 pb-2 text-[10px] tracking-[0.16em] font-bold text-[var(--txt)]">
              <CandlestickChart size={12} className="text-[var(--cyan)]" /> SPOT · 5-MIN
              <span className="mono text-[9.5px] text-[var(--faint)] font-normal normal-case tracking-normal">
          ...
          <div className="col-span-12 xl:col-span-4 space-y-4">
            <div className="panel p-3.5">
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="chip" style={{ color: d.futures.buildup?.includes("Long") ? "var(--long)" : d.futures.buildup?.includes("Short build") ? "var(--short)" : "var(--muted)" }}>
                  {d.futures.buildup ?? "N/A"}
                </span>
              </div>
              <div className="mt-3">
          ...
          {!card && (
            <div className="col-span-12 panel px-5 py-4 mono text-[11.5px] text-[var(--faint)]">
