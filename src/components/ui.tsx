...
export function StatusBadge({ status, pulse }: { status: DataStatus | null | undefined; pulse?: boolean }) {
  ...
  return (
    <span className={`chip ${cls[s]}`}>
      <span
        ...
        style={{
          background:
            s === "LIVE" ? "var(--long)" : s === "RECENT" ? "var(--cyan)" : s === "STALE" ? "var(--wait)" : s === "PARTIAL" ? "var(--violet)" : "var(--faint)",
        }}
      />
      {s}
    </span>
