...
function Ladder({ card }: { card: Card }) {
  ...
  return (
    <div className="relative mx-1 mt-6 mb-5 h-[3px] rounded bg-[#1a2436]">
      {card.support && (
        <div
...
function PlanCell({ k, v, tone }: { k: string; v: string; tone?: string }) {
  return (
    <div className="panel-inset px-2.5 py-1.5 min-w-0">
    ...
    <div className="panel relative overflow-hidden fade-up">
      ...
      <div className="p-4 pl-5">
        ...
        <div className="flex items-start gap-3">
          {rank != null && (
            <div className="mono text-[26px] font-light text-[var(--faint)] leading-none mt-0.5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <Link
                ...
                className="text-[17px] font-bold tracking-tight hover:text-[var(--cyan)] transition-colors"
              >
                {card.symbol}
              </Link>
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            <ScoreRing score={card.finalScore} size={48} />
          </div>
        </div>
        ...
        <div className="mt-3 flex items-end gap-4 flex-wrap">
          <div>
            ...
            <div className="flex items-center gap-1.5">
              <span className="mono text-[22px] font-bold leading-none">₹{fmtINR(card.ltp)}</span>
        ...
        <div className="mt-3 grid grid-cols-3 sm:grid-cols-6 gap-1.5">
          ...
          <PlanCell k="FUTURES" v={card.futuresLabel ?? "N/A"} tone={card.futuresLabel?.includes("Long") ? "var(--long)" : card.futuresLabel?.includes("Short build") ? "var(--short)" : undefined} />
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-3 mt-2">
          <div className="space-y-2">
            ...
            <div className="panel-inset p-2.5 space-y-1.5 mono text-[11.5px]">
              ...
              <div className="flex justify-between gap-2 border-t border-[var(--line-soft)] pt-1.5">
                <span className="text-[var(--violet)]">OPT SUPPORT</span>
                <span>
                  {card.optionSupport ? (
                    <>
                      {fmtINR(card.optionSupport.strike, 0)}
                      <span className="text-[var(--faint)]"> · OI {fmtCompact(card.optionSupport.oi)}{card.optionSupport.oiChg != null ? ` (${card.optionSupport.oiChg > 0 ? "+" : ""}${fmtCompact(card.optionSupport.oiChg)})` : ""}</span>
                    </>
                  ) : (
                    <span className="text-[var(--faint)]">N/A</span>
                  )}
                ...
                <span>
                  {card.optionResistance ? (
                    <>
                      ...
                      <span className="text-[var(--faint)]"> · OI {fmtCompact(card.optionResistance.oi)}{card.optionResistance.oiChg != null ? ` (${card.optionResistance.oiChg > 0 ? "+" : ""}${fmtCompact(card.optionResistance.oiChg)})` : ""}</span>
                    </>
                  ) : (
                    <span className="text-[var(--faint)]">N/A</span>
                  )}
              ...
              <div className="flex justify-between gap-2 border-t border-[var(--line-soft)] pt-1.5 text-[var(--muted)]">
          ...
          <div className="space-y-2">
            ...
            <div className="panel-inset p-2.5 grid grid-cols-3 gap-1.5">
        ...
        <div className="mt-2.5 flex items-center justify-between">
          ...
          <span className="mono text-[9.5px] text-[var(--faint)]">scanned {new Date(card.scannedAt).toLocaleTimeString("en-IN", { timeZone: "Asia/Kolkata", hour12: false })} IST</span>
        </div>
      </div>
    </div>
  );
