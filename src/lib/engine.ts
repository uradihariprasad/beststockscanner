...
class Engine {
  ...
  private warmPrevDay(today: string) {
    ...
    void mapLimit(this.universe, 5, async (u) => {
      try {
        ...
        const prev = [...candles].reverse().find((c) => c.t < new Date(`${today}T00:00:00+05:30`).getTime());
  ...
  async scan(): Promise<void> {
    ...
    try {
      ...
      const changePct =
        nq?.ltp != null && nq?.prevClose != null && nq.prevClose > 0
          ? ((nq.ltp - nq.prevClose) / nq.prevClose) * 100
          : null;
      nifty = {
        ltp: nq?.ltp ?? null,
        prevClose: nq?.prevClose ?? null,
        open: nq?.open ?? null,
    ...
    for (const u of universe) {
      ...
      if (!st.inCandidates && score != null && score >= this.cfg.enterScore && st.momentum !== "NONE") {
    ...
    for (const [sym, st] of this.symState) if (st.inCandidates) candidateSet.add(sym);
  ...
  private async getDeep(u: UniverseEntry, spot: number | null, istDay: string): Promise<DeepCache> {
    ...
    void spot;
    const next: DeepCache = { fetchedAt: Date.now(), expiry, chain, oiBaseline, futCandles5m };
  ...
  private async deepAnalyse(row: Stage1Row, u: UniverseEntry, ms: MarketStatus): Promise<TradeCard | null> {
    ...
    return {
      ...
      rsAccel: row.rsAccel,
      trend5m: row.trend5m,
      futuresLabel: buildupLabel(row.buildup),
      optionsLabel,
      status: row.status,
  ...
  async getStockDetail(symbol: string): Promise<StockDetail | null> {
    ...
    if (deep?.chain && ltp != null) {
      ...
      const sliced = strikes.slice(Math.max(0, atm - 12), atm + 13).map((s, i2) => {
        ...
        return {
          ...s,
          ceOiChg: s.ceOi != null && b?.ce != null ? s.ceOi - b.ce : null,
          peOiChg: s.peOi != null && b?.pe != null ? s.peOi - b.pe : null,
          isAtm: absIdx === atm,
        };
      });
      chainOut = {
