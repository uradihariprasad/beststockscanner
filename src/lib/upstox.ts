...
export interface ChainStrike {
  ...
  ceIV: number | null;
  peIV: number | null;
  /** change vs session baseline (populated by the engine, never by the API) */
  ceOiChg?: number | null;
  peOiChg?: number | null;
  isAtm?: boolean;
}
  ...
  async optionChain(underlyingKey: string, expiry: string): Promise<ChainStrike[]> {
    ...
    for (const item of data ?? []) {
      ...
      strikes.push({
        ...
        ceOi: numOrNull(ce?.oi),
        peOi: numOrNull(pe?.oi),
...
interface RawChainItem {
  strike_price?: number;
  call_options?: {
    market_data?: { oi?: number; volume?: number; ltp?: number };
